...
export class FlightResultComponent implements OnInit {
  ...
  // };
    ...
    filterByAirlineOne(ailinename: any, val: any) {
      ...
      this.filterresult = this.filterresult.filter(row => row.Segments.Segment[0].AirlineCode == ailinename);
    }
   ...
   this.taxserv=parseFloat(this.fareBreakup[0].Fare.Tax)
    ...
    myOnChange(from: any,to:any) {
      ...
      if (this.serchData.Request.JourneyType === "return") {
        ...
        this.filterresult = JSON.parse(localStorage.getItem('filterresultOrg'));
        this.filterresult = this.filterresult.filter(row => row.Fare.PublishedPrice >= from && row.Fare.PublishedPrice <= to);
      }
    }
    // myOnChangeR(avl: any) {
    //   
    //   if (this.serchData.Request.JourneyType === "return")
    //     this.filterresultR = JSON.parse(localStorage.getItem('filterresultROrg'));
    //     this.filterresultR = this.filterresultR.filter(row => row.Fare.PublishedPrice >= avl.from && row.Fare.PublishedPrice <= avl.to);
    ...
    MailTicket(SegmentKey:any)
    {
      ...
      this.model.txtEmail = ((document.getElementById("segMailID") as HTMLInputElement).value);;
